...
import { ArrowLeft, Calendar, User, AlertCircle, CheckCircle, Clock, XCircle } from "lucide-react"
import Link from "next/link"
import StatusUpdateForm from "@/components/status-update-form"
import AdminResponseForm from "@/components/admin-response-form"
          ...
          <div className="flex items-center py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Manage and respond to student grievance</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{grievance.student.full_name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-500">Email</label>
                    <p className="font-medium">{grievance.student.email}</p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-500">Student ID</label>
                    <p className="font-medium">{grievance.student.student_id}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>History of all updates and responses for this grievance</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      .map((update) => (
                        <div
                          ...
                          className={`p-4 rounded-lg border ${
                            update.is_internal ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"
                          }`}
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className="font-medium">{update.admin?.full_name || "System"}</span>
          ...
          <div className="space-y-6">
            <StatusUpdateForm grievanceId={grievance.id} currentStatus={grievance.status} />
            <AdminResponseForm grievanceId={grievance.id} />
          </div>
        </div>
      </main>
    </div>
